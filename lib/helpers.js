...
function _currencyString(theNum){
...
function _objToString(obj) {
    var str = '';
    for(var key in obj) {
      if(obj.hasOwnProperty(key) && key != 'timestamp') str += (key + ': ' + obj[key] + '\t');
      if(obj.hasOwnProperty(key) && key == 'timestamp') str += (obj[key] + '\t');
    }
    return str.slice(0,-1);
  }

// returns largerObject keys common to both objects
function _filterObject(smallerObject, largerObject) {
  let result = true;
  let filteredLargerObject = {};
    for(key in smallerObject){
    if(!largerObject.hasOwnProperty(key)){
      console.log(`larger object missing property '${key}'`);
      result = false;
    }
    if(result) filteredLargerObject[key] = largerObject[key];
  }
  if(result) return filteredLargerObject;
  return result;
}

function _listKeys(obj){
  for (var key in obj) {
